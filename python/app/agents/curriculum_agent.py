...
from app.core.knowledge_graph import KnowledgeGraph, build_sample_math_graph
from app.core.spaced_repetition import SpacedRepetition, ReviewItem
...
class CurriculumAgent(BaseAgent):
    ...
    async def _handle_mastery_update(self, event: Event) -> None:
        ...
        quality = event.data.get("quality_q")
        if quality is None:
        ...
        if db is not None:
            old_schedule = await ReviewScheduleDAO.get_by_user_knowledge(
                db=db,
            ...
            if old_schedule:
                review_item.easiness_factor = old_schedule.ef
                review_item.interval_days = old_schedule.interval_days
                review_item.repetition = old_schedule.repetition
                review_item.last_review = old_schedule.last_review_at
                review_item.next_review = old_schedule.next_review_at

        self.sr.review(review_item, int(quality))
        ...
        if db is not None:
            await ReviewScheduleDAO.upsert_schedule(
                ...
                user_id=int(learner_id),
                knowledge_id=int(knowledge_id),
                ef=review_item.easiness_factor,
                interval_days=int(review_item.interval_days),
                repetition=review_item.repetition,
                last_review_at=review_item.last_review,
                next_review_at=review_item.next_review,
            )
