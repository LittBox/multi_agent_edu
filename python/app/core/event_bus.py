...
class EventBus:
    ...
    def __init__(self) -> None:
        ...
        self._event_history: list[Event] = [] # 事件溯源：记录所有事件，支持回放和分析
        self._lock = asyncio.Lock() # 保护事件历史记录的并发访问

    def subscribe(self, event_type: EventType, handler: EventHandler) -> None:
        """订阅某类事件。"""
        if event_type not in self._subscribers: # 初始化事件类型的订阅列表
            self._subscribers[event_type] = []  # 关键设计：每个事件类型维护一个订阅者列表
        self._subscribers[event_type].append(handler) # 将handler添加到订阅列表中
